...
CHECKPOINT = '/users/ogal/nmashall/dino-state-predictor/DinoAttentionHead/ckp/epoch_60.pt' # withput normalization

VIDEO_PATH  = '/users/ogal/nmashall/dino-state-predictor/DinoAttentionHead/videos/eval_video.mp4'
...
model = DinoTransformerHead(          # ← was DinoMlp — fixed
    action_dim=7,
    state_dim=7,
    hidden_dim=ckpt['hidden_dim'],     # ← read from checkpoint, not hardcoded
    n_heads=ckpt['n_heads'],
    n_enc_layers=ckpt['n_enc_layers'],
    n_dec_layers=ckpt['n_dec_layers'],
).to(device)
